<?php
	echo $this->Form->create('Usuario' , array('action' => 'login', 'class' => 'form with-margin'));
?>
		<?php
		...
		<?php
			if(Configure::read('Admin.Usuario.remember_me')){
				echo $this->Form->input('remember_me',array('label'=> array('style'=>'display:inline','text' => ' Manter-me conectado'), 'class' => 'mini-switch', 'div' => 'input-height', 'type' => 'checkbox', 'autocomplete' => 'off','value' => '1'));
			}
			echo $this->Html->link(__('Esqueci minha senha', true), array('action' => 'recoverpass'));
		 ?>
		 </div>
		 <?php
