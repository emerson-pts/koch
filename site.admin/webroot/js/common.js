...
(function($)
{
	...
	$.fn.addTemplateSetup(function()
	{
		...
		this.find('.js-tabs').initTabs();
   		...
   		this.find('.modal-link').click(function(event){
			// Prevent link opening
			event.preventDefault();
			
			params = {
				title: ($(this).hasClass('with-tip') ? this.titleBck : this.title),
				url: this.href,
				loadingMessage: 'Carregando...'
			};
			
			var options = new Array('modalClass', 'border', 'draggable', 'resizable', 'scrolling', 'closeButton', 'minHeight', 'minWidth', 'maxHeight', 'maxWidth', 'height', 'width', 'resize');	
			for(option in options){
				if($(this).attr(options[option])){
					params[options[option]] = $(this).attr(options[option]);
				}
			}
	...
	$.fn.blockMessage = function(message, options)
	{
		...
		this.each(function(i)
		{
			...
			id = 'message_' + Math.floor(Math.random()*10000);
			var finalMessage = (typeof message == 'object') ? '<ul animated="' + settings.animate + '" id="' + id + '" class="'+messageClass+'"><li>'+message.join('</li><li>')+'</li></ul>' : '<p animated="' + settings.animate + '" id="' + id + '" class="'+messageClass+'">'+message+'</p>';
		...
		jQuery('#' + id).click(function(){
			if(jQuery(this).attr('animated') == 'true'){
				jQuery(this).foldAndRemove();
			}else{
				jQuery(this).remove();
			}
		});
