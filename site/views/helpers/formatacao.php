...
class FormatacaoHelper extends Helper {
	...
	function priceTags($value){
		if(!preg_match('/([a-z]+\$) *([0-9\.]+)([0-9,]+)/i', $value)){
			return '<span class="currency">'.preg_replace('/\$/', '<span class="currency_dollar">\1</span>', $value).'</span>';
		}else{
			return preg_replace('/([a-z]+)(\$) *([0-9\.]+)([0-9,]+)/ie', '\'<span class="currency">\1<span class="currency_dollar">\2</span></span> <span class="price">\'.number_format(str_replace(\'.\', \'\', \'\3\'), 0, \',\', \'.\').\'</span><span class="price price-cents">\4</span>\'', $value);
		}
	}
...
*/
	...
	function plural($s, $c, $showNumber = true) {
		return ($showNumber == true ? $c . ' ' : '') . ($c != 1 ? Inflector::pluralize($s) : $s);
	}
