			...
			<!-- codigo html galeria -->
				<div class="wrap-galeria">
					...
					<div class="right count_thumbs">
						<span class="sevent f10"><?php echo  'foto 1 de '.$result; ?></span>
					</div>
					<br clear="all" />
					<!-- Count thumbs -->
...
jQuery(function(){
  // slider thumb
	...
	var slider_thumb = jQuery('#slider1-galeria').bxSlider({
	...
	jQuery('#thumb-prev').click(function(){
		slider_thumb.goToPreviousSlide();
		return false;
	});

	jQuery('#thumb-next').click(function(){
	...
	onAfterSlide: function(currentSlide, totalSlides){
		  ...
		  jQuery('.identificador-qtd-img').html('FOTO ' + (currentSlide+1) + ' DE ' + totalSlides);
		  jQuery('.ampliar-galeria a').attr("href", $('.thumbs-galeria a.pager-active img').attr("link-lupa"));
		  jQuery('.creditos-galeria-atual').html('Crédito: ' + $('.thumbs-galeria a.pager-active img').attr("credito_image"));
		  //alert('www');
	    }
	});
	    
	jQuery('#img-prev-big').click(function(){
		slider.goToPreviousSlide();
		if((slider_thumb.getCurrentSlide() + (qtd_thumb-1)) < slider.getCurrentSlide()){
			slider_thumb.goToSlide(slider.getCurrentSlide());
		}else{
			jQuery('.thumbs-galeria a').removeClass('pager-active');
		// assisgn "pager-active" to clicked thumb
		    jQuery('.thumbs-galeria:eq(' + (slider.getCurrentSlide() + qtd_thumb) + ') a').addClass('pager-active');
		}
		return false;
	});

	jQuery('#img-next-big').click(function(){
	...
	jQuery('.thumbs-galeria a').click(function(){
