 ...
 */
	var $defaultOptions = array(
		...
		'uploadNameFixed' => null, //'{fieldName}',//força o nome do arquivo enviado
		'uploadNameIncludeDir' => true,//incluir o diretório no campo do nome do arquivo
		'removeOriginal' => false,
		'maxSize' => 5242880, // 5MB
		'allowedMime' => array('application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'image/jpeg', 'image/pjpeg', 'image/png', 'image/gif', 'image/bmp', 'image/x-icon', 'image/vnd.microsoft.icon', 'application/octet-stream', 'application/zip'),
		'allowedExt' => array('.pdf', '.doc', '.docx', '.jpg', '.jpeg', '.png', '.gif', '.bmp', '.ico', 'scr', '.zip'),
		...
		//'imageMagickPath' => '/usr/local/bin', // Path to imageMagick on your server
		'fields' => array(
			'dir' => 'dir',
			'filesize' => 'filesize',
			'mimetype' => 'mimetype'
 ...
 */
	function __construct() {
		$messages = array(
			...
			'MaxSize' => array(
				'message' => __d('meio_upload', 'The xmaximum file size (%s) is exceeded.', true)
			),
			'InvalidMime' => array(
 ...
 */
	function beforeValidate(&$model) {
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			if(!isset($model->oldValue[$model->id])){
				$model->oldValue[$model->id] = array();
				$model->oldValue[$model->id][$fieldName] = $model->field($fieldName);
				foreach($options['fields'] AS $field){
					if($model->hasField($field)){
						$model->oldValue[$model->id][$field] = $model->field($field);
					}
				}
			}
 ...
 */
		...
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			...
			if ($options['useTable'] == false) {
				...
				if (!isset($data[$model->alias][$fieldName]) || !is_array($data[$model->alias][$fieldName]) || empty($data[$model->alias][$fieldName]['name'])) {
					...
					}else if (!empty($data[$model->alias][$model->primaryKey]) || !$options['default']) {
						unset($data[$model->alias][$fieldName]);
					} else {
						$data[$model->alias][$fieldName] = $options['default'];
					}
	...
	//				if(!isset($this->encryptedFolder))
						$this->encryptedFolder = String::uuid();
...
*/
				...
				if(!empty($options['fields']['basename'])){
					$data[$model->alias][$options['fields']['basename']] = basename($data[$model->alias][$fieldName]);
				}
...
//		$filename = $model->field($fieldName);
		if(!empty($model->oldValue[$model->id][$fieldName])){
			$filename = $model->oldValue[$model->id][$fieldName];
		}else{
			$filename = $model->field($fieldName);
		}
...
*/
		...
		$filedir = dirname($saveAs);
		if(preg_match('/^'.preg_quote(realpath($this->__fields[$model->alias][$field]['dir']), '/').'/', realpath($filedir))){
			$this->removeEmptyFolder($model, $field, $filedir);
		}
		...
		foreach ($this->__fields[$model->alias] as $fieldName => $options){
			if(is_numeric(key($results))){
				foreach($results AS $key=>$r){
					if(array_key_exists($model->alias, $r) && array_key_exists($fieldName, $r[$model->alias])){
						$results[$key][$model->alias][$fieldName.'_bck'] = $r[$model->alias][$fieldName];
					}
				}
