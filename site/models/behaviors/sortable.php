...
class SortableBehavior extends ModelBehavior {
    ...
    function beforeSave(&$model) {
		...
		//Se envio o id, não é registro novo, então não seta posição
...
//        $position = $model->data[$model->name][$field];
    ...
    function fixPosition(&$model, $group_id = null, $custom_order = null){
        extract($this->_config($model));
		$sql = 'UPDATE '.$model->tablePrefix.$model->table.' '.$model->alias.'
				JOIN (SELECT 
					acumulado'.$model->alias.'.id,
					'.($group ? '
						@new_order := IF(@last_group != '.$group.', 1, @new_order + 1) AS new_order,
						@last_group := '.$group.' AS last_group
					' :
					'
						@new_order := @new_order + 1 AS new_order
					'
				).'
			  FROM '.$model->tablePrefix.$model->table.' acumulado'.$model->alias.'
			  JOIN (SELECT @new_order := 0, @last_group := null) r
			'.(!empty($custom_order) || !empty($group) || !empty($model->order) ? 
				'ORDER BY '
				:
				''
			).' '.$group.' '.
			(!empty($group) && (!empty($custom_order) || !empty($model->order)) ? ',' : '').'
			'.(!empty($custom_order) ? ' acumulado'.$custom_order : (!empty($model->order) ? 'acumulado'.$model->order : '')).'
			) acumulado ON (acumulado.id = '.$model->alias.'.id)
			SET 
				'.$model->alias.'.`'.$field.'` = acumulado.new_order
		';
